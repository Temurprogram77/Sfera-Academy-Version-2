...
const Teachers = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'status',
      render: (text) => (
        <span className={`px-3 py-1 text-xs rounded-full ${text === "Faol" ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800"}`}>
          {text}
        </span>
      ),
    },
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 mb-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="text-base font-medium text-gray-700">
            O'qituvchilar soni: <span className="font-bold text-gray-900">{filteredTeachers.length}</span>
          </div>
